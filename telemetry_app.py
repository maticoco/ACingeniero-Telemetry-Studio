...
if not csv_names:
    ...
    # st.stop()  # Removido para permitir renderizado de pestañas independientes
...
if not loaded_laps:
    st.sidebar.error("⚠️ Debes cargar al menos 1 vuelta para analizar datos (la captura sigue disponible).")
    # st.stop()  # Removido para permitir renderizado de pestañas independientes
...
with tab1:
    if not loaded_laps:
        st.warning('No se encontraron sesiones cargadas. Graba una sesión en la pestaña 🔴 Captura en Vivo primero.')
    else:
...
with tab7:
    ...
    with col_start:
        if st.button("▶️ Iniciar Captura", type="primary", use_container_width=True, disabled=st.session_state.is_recording):
            ...
            proc = subprocess.Popen([sys.executable, 'recorder'], env=env, stdout=log_file, stderr=subprocess.STDOUT)
